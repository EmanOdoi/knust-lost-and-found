    ...
    <header className="bg-forest text-white sticky top-0 z-30 shadow-sm">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden sm:flex items-center gap-6 text-sm font-medium">
          <Link to="/" className="hover:text-brass-50 transition">Browse</Link>
          {user && <Link to="/report" className="hover:text-brass-50 transition">Report an item</Link>}
          {user && <Link to="/my-reports" className="hover:text-brass-50 transition">My reports</Link>}
          {user?.role === "admin" && <Link to="/admin" className="hover:text-brass-50 transition">Admin</Link>}
        </nav>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <>
              ...
              <button onClick={handleLogout} className="text-sm font-medium bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-sm transition">
                Log out
              </button>
            </>
          ) : (
            <>
              <Link to="/login" className="text-sm font-medium hover:text-brass-50 transition">Log in</Link>
              <Link to="/register" className="text-sm font-medium bg-brass hover:bg-brass/90 px-3 py-1.5 rounded-sm transition">
                Sign up
              </Link>
            </>
          )}
